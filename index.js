...
dobInput.setAttribute('min', minimumDate());
dobInput.setAttribute('max', maximumDate());
...
const dispEntries = () => {
    ...
    const tableEntries = entries.map((entry) => {
        ...
        const acceptTermsCell = `<td class="bor">${entry.acceptedTermsAndConditions}</td>`;

        const row = `<tr>${name} ${email} ${password} ${dateOfBirth} ${acceptTermsCell}</tr>`;
        return row;
    }).join("\n");
...
const formSubmit = (event) => {
    ...
    const entry = {
        name, email, password, dob: dateOfBirth, acceptedTermsAndConditions 
    }
